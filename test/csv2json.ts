...
export function runTests() {
    ...
    describe('csv2json Usage', () => {
        describe('Default Options', () => {
            ...
            it('should return an empty array when provided an empty csv', () => {
                const json = csv2json(csvTestData.noData);
                assert.deepEqual(json, jsonTestData.noData);
            });

            it('should convert a single basic document from csv to json', () => {
                const json = csv2json(csvTestData.singleDocument);
                assert.deepEqual(json, jsonTestData.singleDocument);
            });

            it('should convert csv containing an array representation to json', () => {
                const json = csv2json(csvTestData.array);
                assert.deepEqual(json, jsonTestData.array);
            });

            it('should convert csv containing a date to json', () => {
                const json = csv2json(csvTestData.date);
                assert.deepEqual(json, [jsonTestData.date]);
            });

            it('should convert csv containing null to json', () => {
                const json = csv2json(csvTestData.null);
                assert.deepEqual(json, jsonTestData.null);
            });

            it('should convert csv containing undefined to json', () => {
                const json = csv2json(csvTestData.undefined);
                jsonTestData.undefined.colorPreference = undefined;
                assert.deepEqual(json, [jsonTestData.undefined]);
            });

            it('should convert csv representing nested json to the correct json structure', () => {
                const json = csv2json(csvTestData.nested);
            ...
            it('should convert csv containing a field with a comma to json', () => {
                const json = csv2json(csvTestData.comma);
            ...
            it('should convert csv containing a field with eol characters to json', () => {
                const json = csv2json(csvTestData.eol);
                assert.deepEqual(json, jsonTestData.eol);
            });

            it('should ignore empty lines and not convert them to json', () => {
                const json = csv2json(csvTestData.eol + '\n');
                assert.deepEqual(json, jsonTestData.eol);
            });

            it('should convert csv containing an assortment of values to json', () => {
                const json = csv2json(csvTestData.assortedValues);
                assert.deepEqual(json, jsonTestData.assortedValues);
            });

            it('should handle an extra new line at the end of the csv', () => {
                const json = csv2json(csvTestData.extraLine);
                assert.deepEqual(json, jsonTestData.assortedValues);
            });

            // Test case for #109
            it('should properly handle the cases involving an empty field value', () => {
                const json = csv2json(csvTestData.csvEmptyLastValue);
                assert.deepEqual(json, jsonTestData.csvEmptyLastValue);
            });

            // Test case for #115
            it('should properly handle quoted empty field values', () => {
                const json = csv2json(csvTestData.quotedEmptyFieldValue);
                assert.deepEqual(json, jsonTestData.quotedEmptyFieldValue);
            });

            // Test case for #149
            it('should properly handle invalid parsed values', () => {
                const json = csv2json(csvTestData.invalidParsedValues);
                assert.deepEqual(json, jsonTestData.invalidParsedValues);
            });

            // Test case for #155
            it('should properly convert empty field values if they occur at the end of the csv', () => {
                const json = csv2json(csvTestData.emptyLastFieldValue);
                assert.deepEqual(json, jsonTestData.emptyLastFieldValue);
            });

            // Test case for #161
            it('should properly convert an empty field value if it occurs at the end of the csv with no EOL at the end', () => {
                const json = csv2json(csvTestData.lastCharFieldDelimiter);
                assert.deepEqual(json, jsonTestData.emptyLastFieldValue);
            });

            // Test case for #161
            it('should properly convert two empty field values if they occur at the end of the csv with no EOL at the end', () => {
                const json = csv2json(csvTestData.emptyLastFieldValueNoEol);
                assert.deepEqual(json, jsonTestData.emptyLastFieldValueNoEol);
            });

            // Test case for #184
            it('should properly convert keys with escaped/nested dots in them', () => {
                const json = csv2json(csvTestData.nestedDotKeys);
                assert.deepEqual(json, jsonTestData.nestedDotKeys);
            });

            // Test case for #184
            it('should properly convert keys with escaped/nested dots in them even when they appear in an array', () => {
                const json = csv2json(csvTestData.nestedDotKeysWithArray);
                assert.deepEqual(json, jsonTestData.nestedDotKeysWithArray);
            });

            // Test case for #184
            it('should properly convert keys with escaped/nested dots in them even when they appear in an expanded, unwound array', () => {
                const json = csv2json(csvTestData.nestedDotKeysWithArrayExpandedUnwound);
                assert.deepEqual(json, jsonTestData.nestedDotKeysWithArrayExpandedUnwound);
            });

            // Test case for #204
            it('should drop any values with empty column headers', () => {
                const json = csv2json(csvTestData.emptyColumns);
                assert.deepEqual(json, jsonTestData.emptyColumns);
            });

            // Test case for #214
            it('should handle quoted fields with nested EOL characters', () => {
        ...
        describe('Error Handling', () => {
            ...
            it('should throw an error if data is not the proper type - no options provided', () => {
                try {
                    csv2json([] as any);
                    throw testFailureError;
                } catch (error) {
            ...
            it('should throw an error if data is not the proper type - options provided', () => {
                try {
                    csv2json([] as any, {});
                    throw testFailureError;
                } catch (error) {
        ...
        describe('Custom Options', () => {
            ...
            it('should wrap first column with crlf line break', () => {
                const json = csv2json(csvTestData.firstColumnWrapCRLF, {
                    delimiter: { eol: '\r\n' }
                });
                assert.deepEqual(json, jsonTestData.firstColumnWrapCRLF);
            });

            it('should strip the excel byte order mark character, if specified, and convert to json', () => {
                const json = csv2json(csvTestData.excelBOM, {
                    excelBOM: true
                });
                assert.deepEqual(json, jsonTestData.assortedValues);
            });

            it('should trim header fields and still convert to json', () => {
                const json = csv2json(csvTestData.trimHeader, {
                    trimHeaderFields: true
                });
                assert.deepEqual(json, jsonTestData.trimmedHeader);
            });

            it('should trim field values and still convert to json', () => {
                const json = csv2json(csvTestData.trimFields, {
                    trimFieldValues: true
                });
                assert.deepEqual(json, jsonTestData.trimmedFields);
            });

            it('should use the provided keys and still convert to json', () => {
                const json = csv2json(csvTestData.assortedValues, {
            ...
            it('should be able to parse CSV with no header line provided and only some of the fields', () => {
                const headerlessCsv = csvTestData.unwindWithSpecifiedKeys.split(options.delimiter.eol)
                        .splice(1)
                        .join(options.delimiter.eol),
                    expectedJson = jsonTestData.unwindWithSpecifiedKeys.map((doc: any) => {
    ...
    describe('csv2json Promise Chain Usage', () => {
        describe('Default Options', () => {
            ...
            it('should still convert an array of json documents', () => {
                const json = csv2json(csvTestData.array);
            ...
            describe('Error Handling', () => {
                it('should still throw an error when not passed data and it should hit the catch block', () => {
                    try {
                        csv2json(null as any);
                    } catch (error) {
                        assert.equal((error as Error).message, `${constants.errors.csv2json.cannotCallOn} null.`);
                ...
                it('should still throw an error the data is of the wrong type and it should hit the catch block', () => {
                    try {
                        csv2json(new Date() as any);
                    } catch (error) {
                        assert.equal((error as Error).message, constants.errors.csv2json.dataCheckFailure);
