...
const Json2Csv = function(options) {
    ...
    function getFieldNameList(data) {
        ...
        return Promise.resolve(deeks.deepKeysFromList(data, { expandArrayObjects: options.expandArrayObjects }));
    }
    ...
    function processRecordFieldDataForExpandedArrayObject(recordFieldValue) {
        if (Array.isArray(recordFieldValue) && !recordFieldValue.length) {
            return options.emptyFieldValue || '';
        } else if (Array.isArray(recordFieldValue) && recordFieldValue.length === 1 && utils.isEmptyField(recordFieldValue[0])) {
            return options.emptyFieldValue || '';
        } else if (Array.isArray(recordFieldValue)) {
            // Since we are expanding array objects, we will want to key in on values of objects.
            // Start by getting all truthy values for the field in the array
            let filteredRecordFieldValue = _.filter(recordFieldValue);

            if (filteredRecordFieldValue.length === 1) {
    ...
    function retrieveRecordFieldData(record, fields) {
        ...
        fields.forEach((field) => {
            ...
            } else if (options.expandArrayObjects) {
