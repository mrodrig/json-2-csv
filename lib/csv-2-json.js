...
var splitLine = function (line) {
    // Enumerate the possible cases for CSV wrapping
    var case1 = addSlashes(options.DELIMITER.WRAP + options.DELIMITER.FIELD + options.DELIMITER.WRAP),
        case2 = addSlashes(options.DELIMITER.FIELD + options.DELIMITER.WRAP),
        case3 = addSlashes(options.DELIMITER.WRAP + options.DELIMITER.FIELD),
    // Utilize the enumerated cases to create a regular expression string that can be used to split the line
        regExStr = '(' + case1 + '|' + case2 + '|' + case3 + ')',
    // Create the regular expression
        regEx = new RegExp(regExStr, 'g'),
    // Split the line by the regular expression
        splitLine = line.split(regEx);

    // Return all values that are not the intermediates
    return _.reject(splitLine, function (value) {
