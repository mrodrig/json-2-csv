...
var convertData = function (data, keys) {
    ...
    return _.reduce(keys, function (output, key) {
        ...
        return output.concat(convertField(fieldData, key));
    }, []);
};
 ...
 * @param output
 */
var convertField = function (value, key) {
    if (_.isArray(value)) { // We have an array of values
        ...
        if (key.indexOf('.') === -1) {
            return options.DELIMITER.WRAP + JSON.stringify(value) + options.DELIMITER.WRAP; // Return object as string for top level objects
        }
